...
def execute_command(shell_id: str, command: str, waittime: int, timeout: int) -> str:
    ...
    if _IS_ROOT and not session.sudo and command.strip().startswith("sudo"):
        return (
            "This shell session does not have sudo privileges, so this command would "
            "fail. Create a new shell session with sudo=true and run it there."
        )
